...
def main(subjects: list[str]) -> None:
    ...
    for subject in subjects:
        ...
        # Pair segments by phase and index. Both branches segment the same phase of
        # the same recording, so identical indices refer to the same 60 seconds of
        # wall-clock time — which is what makes this a genuinely paired comparison.
        merged = ecg.merge(ppg, on=["subject", "phase", "segment"],
                           suffixes=("_ecg", "_ppg"))
